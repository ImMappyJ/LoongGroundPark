...
#define IIC_SCK_0  GPIOA->BRR = 0x0080       // ??SCK??????,??PA7
#define IIC_SCK_1  GPIOA->BSRR = 0x0080       // ??SCK??????,??PA7
#define IIC_SDA_0  GPIOA->BRR = 0x0100       // ??SDA??????,??PA8
#define IIC_SDA_1  GPIOA->BSRR = 0x0100       // ??SDA??????,??PA8

const unsigned char  *point2;
unsigned char ACK2=0;
const unsigned char  OLED_init_cmd2[25]=			//SH1106
{
...
const unsigned char  picture_tab2[]={
/*------------------------------------------------------------------------------
...
/*******ê±?ó3?ê??¯******************/
void SYS_init2(unsigned char PLL)
{
...
void IIC_write2(unsigned char date)
{
...
void OLED_send_cmd2(unsigned char o_command)
  {
    
    IIC_start2();
    IIC_write2(0x00); 
    IIC_write2(o_command);
    IIC_stop2();
    
  }
void OLED_send_data2(unsigned char o_data)
  { 
    IIC_start2();
    IIC_write2(0x40);
    IIC_write2(o_data);
...
void Column_set2(unsigned char column)
  {
	column+=OLED_COLUMN_OFFSET;
    OLED_send_cmd2(0x10|(column>>4));    //éè??áDµ??·????
    OLED_send_cmd2(0x00|(column&0x0f));   //éè??áDµ??·µí??  
    	 
  }
void Page_set2(unsigned char page)
  {
    OLED_send_cmd2(0xb0+page);
  }
...
void OLED_init2(void)
  {
    ...
    for(i=0;i<25;i++)
      {
        OLED_send_cmd2(OLED_init_cmd2[i]);
      }
  }
...
{
	OLED_send_cmd2(0xb0+y);

	OLED_send_cmd2(((x&0xf0)>>4)|0x10);  //???????? ?????
	OLED_send_cmd2((x&0x0f)|0x01);
}
/**
...
void  OLED_Show_number2(u8 x,u8  y,u8  number )
{
	uint32_t i=0;
	if(number>9){
	OLED_SetPos2(15*(x-1)+1,y-1+(y-1));
	for(i=0;i<8;i++)
	{
		OLED_send_data2(number_array2[number/10*16+i]);
	}
	OLED_SetPos2(15*(x-1)+1,y-1+1+(y-1));
	for(i=8;i<16;i++)
	{
		OLED_send_data2(number_array2[number/10*16+i]);
	}
	OLED_SetPos2(15*(x-1)+1+16,y-1+(y-1));
	for(i=0;i<8;i++)
	{
		OLED_send_data2(number_array2[number%10*16+i]);
	}
	OLED_SetPos2(15*(x-1)+1+16,y-1+1+(y-1));
	for(i=8;i<16;i++)
	{
		OLED_send_data2(number_array2[number%10*16+i]);
	}
	...
	for(i=0;i<8;i++)
	{
		OLED_send_data2(number_array2[number*16+i]);
	}
		OLED_SetPos2(15*(x-1)+1,y-1+1+(y-1));
	for(i=8;i<16;i++)
	{
...
void OLED_Show_chinese2(u8 x,u8  y,u8  number )
{
	...
	for(i=0;i<8;i++)
	{
		OLED_send_data2(chinese_array2[number*2][i]);
	}
	OLED_SetPos2(15*(x-1)+1+8,y-1+(y-1));
	for(i=8;i<16;i++)
	{
		OLED_send_data2(chinese_array2[number*2][i]);
	}
	...
	for(i=0;i<8;i++)
	{
		OLED_send_data2(chinese_array2[number*2+1][i]);
	}
	OLED_SetPos2(15*(x-1)+1+8,y-1+1+(y-1));
	for(i=8;i<16;i++)
	{
		OLED_send_data2(chinese_array2[number*2+1][i]);
	}
...
void OLED_Show_string2(u8 x,u8  y,u8  number )
{
	...
	for(i=0;i<8;i++)
	{
		OLED_send_data2(string_array2[number*2][i]);
	}
	OLED_SetPos2(15*(x-1)+1+8,y-1+(y-1));
	for(i=8;i<16;i++)
	{
	...
	for(i=0;i<8;i++)
	{
		OLED_send_data2(string_array2[number*2+1][i]);
	}
	OLED_SetPos2(15*(x-1)+1+8,y-1+1+(y-1));
	for(i=8;i<16;i++)
	{
...
void OLED_Show_picture2(u8 x,u8  y,u8  number)
{
	...
	for(i=0;i<8;i++)
	{
		OLED_send_data2(picture_arrar2[j*2][i]);
	}
	OLED_SetPos2(15*(x-1)+1+8,y-1+(y-1));
	for(i=8;i<16;i++)
	{
	...
	for(i=0;i<8;i++)
	{
		OLED_send_data2(picture_arrar2[j*2+1][i]);
	}
	OLED_SetPos2(15*(x-1)+1+8,y-1+1+(y-1));
	for(i=8;i<16;i++)
	{
	...
	j++;
		OLED_SetPos2(15*(x+1-1)+1,y-1+(y-1));
	for(i=0;i<8;i++)
	{
		OLED_send_data2(picture_arrar2[j*2][i]);
	}
	OLED_SetPos2(15*(x+1-1)+1+8,y-1+(y-1));
	for(i=8;i<16;i++)
	{
		OLED_send_data2(picture_arrar2[j*2][i]);
	}
	OLED_SetPos2(15*(x+1-1)+1,y-1+1+(y-1));
	for(i=0;i<8;i++)
	{
		OLED_send_data2(picture_arrar2[j*2+1][i]);
	}
	OLED_SetPos2(15*(x+1-1)+1+8,y-1+1+(y-1));
	for(i=8;i<16;i++)
	{
		OLED_send_data2(picture_arrar2[j*2+1][i]);
	}
	...
	j++;
		OLED_SetPos2(15*(x-1)+1,y+1-1+(y+1-1));
	for(i=0;i<8;i++)
	{
		OLED_send_data2(picture_arrar2[j*2][i]);
	}
	OLED_SetPos2(15*(x-1)+1+8,y+1-1+(y+1-1));
	for(i=8;i<16;i++)
	{
		OLED_send_data2(picture_arrar2[j*2][i]);
	}
	OLED_SetPos2(15*(x-1)+1,y+1-1+1+(y+1-1));
	for(i=0;i<8;i++)
	{
		OLED_send_data2(picture_arrar2[j*2+1][i]);
	}
	OLED_SetPos2(15*(x-1)+1+8,y+1-1+1+(y+1-1));
	for(i=8;i<16;i++)
	{
		OLED_send_data2(picture_arrar2[j*2+1][i]);
	}
...
j++;
		OLED_SetPos2(15*(x+1-1)+1,y+1-1+(y+1-1));
	for(i=0;i<8;i++)
	{
		OLED_send_data2(picture_arrar2[j*2][i]);
	}
	OLED_SetPos2(15*(x+1-1)+1+8,y+1-1+(y+1-1));
	for(i=8;i<16;i++)
	{
		OLED_send_data2(picture_arrar2[j*2][i]);
	}
	OLED_SetPos2(15*(x+1-1)+1,y+1-1+1+(y+1-1));
	for(i=0;i<8;i++)
	{
		OLED_send_data2(picture_arrar2[j*2+1][i]);
	}
	OLED_SetPos2(15*(x+1-1)+1+8,y+1-1+1+(y+1-1));
	for(i=8;i<16;i++)
	{
		OLED_send_data2(picture_arrar2[j*2+1][i]);
	}
		...
		j++;
		OLED_SetPos2(15*(x-1)+1,y+1+1-1+(y+1+1-1));
	for(i=0;i<8;i++)
	{
		OLED_send_data2(picture_arrar2[j*2][i]);
	}
	OLED_SetPos2(15*(x-1)+1+8,y+1+1-1+(y+1+1-1));
	for(i=8;i<16;i++)
	{
		OLED_send_data2(picture_arrar2[j*2][i]);
	}
	OLED_SetPos2(15*(x-1)+1,y+1+1-1+1+(y+1+1-1));
	for(i=0;i<8;i++)
	{
		OLED_send_data2(picture_arrar2[j*2+1][i]);
	}
	OLED_SetPos2(15*(x-1)+1+8,y+1+1-1+1+(y+1+1-1));
	for(i=8;i<16;i++)
	{
		OLED_send_data2(picture_arrar2[j*2+1][i]);
	}
			...
			j++;
		OLED_SetPos2(15*(x+1-1)+1,y+1+1-1+(y+1+1-1));
	for(i=0;i<8;i++)
	{
		OLED_send_data2(picture_arrar2[j*2][i]);
	}
	OLED_SetPos2(15*(x+1-1)+1+8,y+1+1-1+(y+1+1-1));
	for(i=8;i<16;i++)
	{
		OLED_send_data2(picture_arrar2[j*2][i]);
	}
	OLED_SetPos2(15*(x+1-1)+1,y+1+1-1+1+(y+1+1-1));
	for(i=0;i<8;i++)
	{
		OLED_send_data2(picture_arrar2[j*2+1][i]);
	}
	OLED_SetPos2(15*(x+1-1)+1+8,y+1+1-1+1+(y+1+1-1));
	for(i=8;i<16;i++)
	{
		OLED_send_data2(picture_arrar2[j*2+1][i]);
	}
			...
			j++;
		OLED_SetPos2(15*(x-1)+1,y+1+1+1-1+(y+1+1+1-1));
	for(i=0;i<8;i++)
	{
		OLED_send_data2(picture_arrar2[j*2][i]);
	}
	OLED_SetPos2(15*(x-1)+1+8,y+1+1+1-1+(y+1+1+1-1));
	for(i=8;i<16;i++)
	{
		OLED_send_data2(picture_arrar2[j*2][i]);
	}
	OLED_SetPos2(15*(x-1)+1,y+1+1+1-1+1+(y+1+1+1-1));
	for(i=0;i<8;i++)
	{
		OLED_send_data2(picture_arrar2[j*2+1][i]);
	}
	OLED_SetPos2(15*(x-1)+1+8,y+1+1+1-1+1+(y+1+1+1-1));
	for(i=8;i<16;i++)
	{
		OLED_send_data2(picture_arrar2[j*2+1][i]);
	}
				...
				j++;
		OLED_SetPos2(15*(x+1-1)+1,y+1+1+1-1+(y+1+1+1-1));
	for(i=0;i<8;i++)
	{
		OLED_send_data2(picture_arrar2[j*2][i]);
	}
	OLED_SetPos2(15*(x+1-1)+1+8,y+1+1+1-1+(y+1+1+1-1));
	for(i=8;i<16;i++)
	{
		OLED_send_data2(picture_arrar2[j*2][i]);
	}
	OLED_SetPos2(15*(x+1-1)+1,y+1+1+1-1+1+(y+1+1+1-1));
	for(i=0;i<8;i++)
	{
		OLED_send_data2(picture_arrar2[j*2+1][i]);
	}
	OLED_SetPos2(15*(x+1-1)+1+8,y+1+1+1-1+1+(y+1+1+1-1));
	for(i=8;i<16;i++)
	{
		OLED_send_data2(picture_arrar2[j*2+1][i]);
	}
